...
"-+3" => false
"95a54e53" => false
It is intended for the problem statement to be ambiguous. You should gather all
requirements up front before implementing one. However, here is a list of
...
bool isNumber(string s) {
    ...
    // when 'e' exists
    if (e_pos.size() !=
        0) {  // 'e' can't be at the first position or the last position
        ...
        if (plus_pos.size() == 1 and minus_pos.size() == 1 and
            ((plus_pos[0] - e_pos[0]) * (minus_pos[0] - e_pos[0]) > 0)) {
            return false;
        } else if (plus_pos.size() == 2 and
                   ((plus_pos[0] - e_pos[0]) * (plus_pos[1] - e_pos[0]) >
                    0)) {  // if there're 2 '+'s
            // they must be on different sides of 'e'
            return false;
        } else if (minus_pos.size() == 2 and
                   ((minus_pos[0] - e_pos[0]) * (minus_pos[1] - e_pos[0]) >
                    0)) {
            return false;
        }

        // '-' '+' pos must be either the first location or immediently after
        // 'e'
        ...
        if (plus_pos.size() != 0 or minus_pos.size() != 0) {
            ...
            int pos_to_check =
                (plus_pos.size() != 0) ? plus_pos[0] : minus_pos[0];
            if (pos_to_check != 0) {
                return false;
            }
            ...
            if (not isdigit(s[pos_to_check + 1])) {
                ...
                if (s[pos_to_check + 1] != '.' or
                    not isdigit(s[pos_to_check + 2])) {
                    return false;
                }
            }
        }
...
int main() {
    vector<string> test_cases{
        "0",   " 0.1", "abc",    "1 a",     "2e10",  "-90e3", "1e",
        "e3",  "6e-1", "99e2.5", "53.5e93", "--6",   "-+3",   "95a54e53",
        " ",   ".",    ".e",     ".e1",     "1.e1",  "+",     "-",
        "4e+", "e",    "+.8",    "+.8e10",  "+.e10", "-e58"};

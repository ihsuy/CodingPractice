...
void increment_counter(int& unique_count,
                       vector<int>& letter_count,
                       const char& n) {
    if (letter_count[n] == 0) {  // never seen this letter, so its unique
        unique_count++;
        letter_count[n] = 1;
    } else {
        letter_count[n]++;
...
void decrement_counter(
    int& unique_count,
    vector<int>& letter_count,
    const char& n) {  // decrement counter, decrement unique_count if its
                      // counter becomes 0
    letter_count[n]--;
...
int lengthOfLongestSubstringKDistinct(const string& s, int k) {
    ...
    while (j < s.length()) {  // loop ends when rhs of the window exceeds array
                              // boundary
        if (unique_count != k or
            letter_count[s[j]] !=
                0) {  // if the substring contain less than k character
            // or its not going to contain more than k characters
            // move rhs of the window to right, until this condition doesn't
            // hold
            increment_counter(unique_count, letter_count, s[j]);
            j++;
        } else {  // if the current substring already contain k unique
                  // characters
            ...
            while (unique_count == k) {  // move lhs of window to the right,

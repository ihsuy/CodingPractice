...
vector<pair<int, int>> twoSum(const vector<int>& nums, const int& low, const int& target)
{
...
vector<vector<int>> fourSum(vector<int>& nums, const int& target) {
    ...
    {
        if ((nums[i] + nums[i] + nums[i + 1] + nums[i + 2]) > target) break;
        if ((nums[i] + nums[nums.size() - 1] + nums[nums.size() - 2] + nums[nums.size() - 3]) >= target)
        {
            for (int j = i + 1; j < nums.size() - 2;)
            {
                ...
                if ((nums[i] + nums[j] + nums[nums.size() - 1] + nums[nums.size() - 2]) >= target)
                {
                    ...
                    for (auto& item : temp)
                    {
                        s.push_back({nums[i], nums[j], item.first, item.second});
                    }
                }
                j = r[nums[j]].second + 1;
            }

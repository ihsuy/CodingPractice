...
inline long long two(long long n) {
    return 1 << n;
}
inline long long test(long long n, long long b) {
    return (n >> b) & 1;
}
inline void set_bit(long long& n, long long b) {
    n |= two(b);
}
inline void unset_bit(long long& n, long long b) {
    n &= ~two(b);
}
inline long long last_bit(long long n) {
    return n & (-n);
}
...
void ao_helper(const string& num,
               const long long& i,
...
vector<string> addOperators(string num, int target) {
    if (num.length() == 0) {
        ...
        return (num[0] - '0' == target) ? vector<string>{num}
                                        : vector<string>{};
    }
    ...
    void dfs(const string& S,
        ...
        for (size_t i = start; i < S.size(); ++i) {
            ...
            if (start == 0) {
                ...
                dfs(S, i + 1, expr, expr_len, -val, cur - val, ans);
                expr[last_expr_len] = '*';
                dfs(S, i + 1, expr, expr_len, val * last,
                    cur - last + val * last, ans);
            }
        }
    }

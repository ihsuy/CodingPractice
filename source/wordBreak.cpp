...
void old_wordBreak_helper(const string& prefix, const string& sentence,
                          const string& s, const int& i, const unordered_set<string>& wordDict,
                          list<string>& result, unordered_map<int, string>& buffer)
{
	...
	if (i == s.length() - 1)
	{
		...
		{
			result.push_back(sentence + ((sentence.length() == 0) ? "" : " ") + new_prefix);
		}
		return;
	}

	if (wordDict.find(new_prefix) == wordDict.end())
	{	// if the word is NOT found in dictionary, continue
		old_wordBreak_helper(new_prefix, sentence, s, i + 1, wordDict, result, buffer);
	}
	else
	{	// if this word appears in the dictionary
		...
		// add to sentence
		old_wordBreak_helper("", sentence + ((sentence.length() == 0) ? "" : " ") + new_prefix,
...
vector<string> wordBreak_helper(const string& s, const unordered_set<string>& wordDict,
                                unordered_map<string, vector<string>>& buffer)
{
	...
	for (string word : wordDict)
	{
		...
		if (word == token)
		{
			auto remain = wordBreak_helper(s.substr(token.size()), wordDict, buffer);
			for (auto& piece : remain)
			{
				result.push_back(token + ((piece.length() == 0) ? "" : " ") + piece);
			}
		}
	}

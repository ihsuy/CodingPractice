...
ListNode* removeElements(ListNode* head,
                         int val) {  // there are 2 big cases to handle
    ...
    // case 1
    while (head != nullptr and
           head->val == val) {  // delete head while moving forwared
        auto hn = head->next;
        delete head;
        head = hn;
    }
    if (head == nullptr) {  // no more element? return emptry linkedlist
        return nullptr;
    }
    ...
    auto temp = head;
    while (head->next != nullptr) {  // go ahead until we reach the end
        while (head->next != nullptr and
               (head->next->val !=
                val)) {  // go ahead until we reach the end or find val
            head = head->next;
        }
...
ListNode* removeElements2(ListNode* head, int val) {  // more concise

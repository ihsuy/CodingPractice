...
struct divisionNode {
    ...
    void add_neighbour(divisionNode* node,
                       const double& val) {  // create mutual edge
        values[node] = val;
...
vector<double> calcEquation(vector<pair<string, string>> equations,
                            ...
                            vector<pair<string, string>>
                                queries) {  // note that this is a graph problem
    ...
    for (int i = 0; i < equations.size(); ++i) {
        if (vars.count(equations[i].first) == 0) {
            ...
            if (vars.count(equations[i].second) != 0) {
                vars[equations[i].first]->add_neighbour(
                    vars[equations[i].second], values[i]);
            } else {
                divisionNode* n2 = new divisionNode();
            ...
            for (int j = 0; j < current_node->neighbours.size(); ++j) {
                if (visited.count(current_node->neighbours[j]) == 0) {
                    todo.push({current_node->neighbours[j],
                               current_val *
                                   current_node
                                       ->values[current_node->neighbours[j]]});
...
int main() {
    ...
    vector<pair<string, string>> queries{{"b", "a"}, {"a", "f"}, {"f", "f"},
                                         {"e", "e"}, {"c", "c"}, {"a", "c"},
                                         {"f", "e"}};

...
legibility) P   A   H   N A P L S I I G Y   I   R And then read line by line:
"PAHNAPLSIIGYIR"
...
string zigzagnize(const string& s, const int& k) {
    if (k == 1) {  // if k is 1 then there's no zigzaging
        return s;
    }
    ...
    for (int p = 0; p < k; ++p) {  // iterate over first k letter
        for (int i = p, c = 0, step = 1; i < slen;
             i += step, c++) {  // c is a counter used to determine whether
                                // should i jump up or down
            if (step != 0) {    // if i jumps for 0 steps, ofc we don't appended

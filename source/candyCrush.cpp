...
board[i][j] = 0 represents that the cell at position (i, j) is empty. The given
board represents the state of the game following the player's move. Now, you
need to restore the board to a stable state by crushing candies according to the
following rules:
...
you need to repeat the above steps. If there does not exist more candies that
can be crushed (ie. the board is stable), then return the current board. You
need to perform the above rules until the board becomes stable, then return the
current board.
    ...
    for (int r = 0; r < rows; ++r) {
        for (int c = 0; c < cols; ++c) {
            if (board[r][c] != 0) {
                ...
                while (i < cols) {
                    if (board[r][i] != n and board[r][i] != -n) {
                        break;
                    }
                    ct++;
                    i++;
                ...
                ct = 1;
                while (j < rows) {
                    if (board[j][c] != n and board[j][c] != -n) {
                        break;
                    }
                    ct++;
                    j++;

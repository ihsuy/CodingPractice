...
ListNode* reverseBetween(ListNode* head,
                         int m,
                         int n) {  // Note: m starts from 1 (not 0)
    ...
    if (m != 1) {  // this process seperate out the part 1(unreversed portion 1)
        for (int i = 1; i < m - 1; ++i) {
            head = head->next;
        }

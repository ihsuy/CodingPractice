...
string S, then we will replace that occurrence of x with y.  If not, we do
nothing.
...
Using another example on S = "abcd", if we have both the replacement operation i
= 0, x = "ab", y = "eee", as well as another replacement operation i = 2, x =
...
string findReplaceString(string S,
    ...
    sort(inputs.begin(), inputs.end(),
         [](pair<int, pair<string, string>>& p1,
    ...
    for (int i = 0; i < inputs.size(); ++i) {
        ...
        if (validSource) {
            S.replace(left, inputs[i].second.first.length(),
                      inputs[i].second.second);
            offset += inputs[i].second.second.length() -
                      inputs[i].second.first.length();
        }
    }
...
string findReplaceString2(string S,
                          vector<int>& idx,
                          vector<string>& src,
                          vector<string>& tgs) {

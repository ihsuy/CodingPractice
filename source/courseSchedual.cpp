...
{	// first recognize this is a graph problem
	...
	{
		for (int j = 0; j < course_list[base_courses[i]].upperLevelCourses.size(); ++j)
		{
			int upper_course = course_list[base_courses[i]].upperLevelCourses[j];
			course_list[upper_course].prerequisite_count--;
			if (course_list[upper_course].prerequisite_count == 0)
			{	// new base courses created, 

...
original array. Every element of the array should be a character (not int) of
length 1. After you are done modifying the input array in-place, return the new
length of the array.
*/
...
int encode(int n,
           vector<char>& s,
           const int& i,
           int& j) {  // works for n larger than 0
    ...
    if (n == 1) {  // when n is 1 don't encode its length
        j++;
        return 0;
    }
...
int compress(vector<char>& s) {
    ...
    int i = 1, j = 1;
    for (; i < s.size() + 1; ++i) {
        if (i == s.size() or s[i] != s[prev]) {  // number of duplicates
            int count = i - prev;
            // encode
            encode(count, s, i, j);

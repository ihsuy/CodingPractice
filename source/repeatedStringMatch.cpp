...
int repeatedStringMatch(const string& A, const string& B) {
    ...
    while (ai != alen and bi != blen) {
        ...
        while (ai != temp_ai - 2 and bi < blen and
               A[ai] == B[bi]) {  // wrap searching in A
            ai = (ai + 1) % alen;
            bi++;

            if (temp_ai - 2 < 0 and
                ai == 0) {  // handle loop breaking corner case
        ...
        if (A[ai] == B[bi]) {  // we found a good starting point
            break;
        }
    }
    ...
    if (ai != 0) {  // a piece of A detected in the beginning of B
        // repeat once
        result++;
    }

...
int firstMissingPositive(vector<int> nums) {  // preprocessing
    bool seen_one = false;

    for (int i = 0; i < nums.size();
         ++i) {  // set every invalid number (number that are less than 1)
        // to 1
    ...
    if (not seen_one) {  // if no 1 was found
        // result is 1
        return 1;
    }
    ...
    // in other words, we use negative sign to indicate this number exists in
    // nums
    ...
    for (int i = 0; i < nums.size();
         ++i) {  // we still need to check the for number
        // though its previously marked negative
        int idx = (nums[i] < 0) ? -nums[i] - 1 : nums[i] - 1;

        if (idx < nums.size() and nums[idx] > 0) {  // flip sign indicate "seen"
            nums[idx] = -nums[idx];
        }
    }

    for (int i = 0; i < nums.size();
         ++i) {  // find the first positive entry and that's the answer
        if (nums[i] > 0) {
            return i + 1;
        }
    }
...
int main() {
    vector<int> v{3, 1, -1, 1, 12, 5, 1, 1, 4, 1, 2, 0};

    cout << "firstMissingPositive linear extra space: "
         << firstMissingPositive_linearSpace(v) << '\n';
    cout << "firstMissingPositive inplace           : "

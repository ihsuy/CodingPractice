...
vector<int> polyPower(const vector<int>& p, const int& n)
{	// taking polynomal p to the n-th power (n>=0)
	if (n == 0)
	{
		return {1};
	}
...
vector<int> polyComposition(const vector<int>& p1, const vector<int>& p2)
{	// insert p1 to p2
	vector<int> result = {0};

...
bool matchMove(int& left,
               int& right,
               const string& s,
               vector<string>& dict) {  // match word and slide window if longer
                                        // word is found
    ...
    for (int i = 0; i < dict.size(); ++i) {
        ...
        if (word[0] == initial_letter and
            word.length() > width) {  // if word length is shorter than width,
                                      // the word included
...
string addBoldTag(string s, vector<string>& dict) {
    ...
    for (int i = 0, j = 0; i < s.length(); ++i) {  // try to match word by word
        if (matchMove(i, j, s, dict)) {
            if (result.empty()) {
                result.push_back({i, j});
            } else {  // merge ranges
                if (i >= result.back().first and i <= result.back().second and
                    j > result.back().second) {
    ...
    for (int i = 0; i < result.size();
         ++i) {  // seperate string into pieces and add tags
        string p1 = s.substr(prev_end, result[i].first - prev_end);
        string p2 =
            s.substr(result[i].first, result[i].second - result[i].first);
        if (p1.length() != 0) {
            pieces.push_back(p1);
        }
        pieces.push_back("<b>");
    ...
    string p3 =
        s.substr(result.back().second, s.length() - result.back().second);
    if (p3.length() != 0) {
        pieces.push_back(p3);
    }

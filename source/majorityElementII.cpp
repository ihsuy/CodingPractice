...
vector<int> majorityElement(
    vector<int>& nums) {  // since we are look for elements that appear more
                          // than n/3 times
    ...
    for (int i = 0; i < nums.size();
         ++i) {  // this approach can be understood by imagining a bar graph
                 // where the
        ...
        if (h1 == 0 and h2 == 0) {  // both bars have height equal to 0 implies
                                    // all elements we've
            // seen so far had an even distribution in terms of their quantities
            // any new elements found after this point, will become the most
            // dominating one and will be inserted in n1's slot
            n1 = curr;
            h1++;
        } else if (h1 != 0 and
                   h2 == 0) {  // since the second slot is open, insert it there
            if (curr == n1) {
                h1++;
            } else {
                n2 = curr;
